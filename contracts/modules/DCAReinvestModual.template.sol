...
library ReinvestTemplate {
    ...
    function _execute(uint256 amount_, bytes memory data_) internal returns(uint256 , bool success) {
        ReinvestDataStruct memory investData = _decodeData(data_);
        return (amount_, success);
    }
    
    function _unwind(uint256 amount_, bytes memory data_)internal returns (uint256 amount, bool success){
        ReinvestDataStruct memory investData = _decodeData(data_);
